...
Sow::Sow(World* worldPtr, InfoTeller* info)
	: Plant(0, 's',6,13, worldPtr, info)
{}

Organism* Sow::Baby(World* worldPtr, InfoTeller* info)
{
	return new Sow(worldPtr, info);
}

void Sow::action(int positionX, int positionY, int sizeX, int sizeY, char symbol, World& world, InfoTeller& info)
{
	int i = 0;
	bool move = false;
	while(i!=3 && move==false){
